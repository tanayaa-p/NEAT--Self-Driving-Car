...
def main(genomes = [], config = []):
    ...
    while run:
        ...
        while(i < len(cars)):
            ...
            if t>10 and (car.detectCollision(road) or y > world.getBestCarPos()[1] + BAD_GENOME_TRESHOLD or y>y_old or car.vel < 0.1): #the t is used to avoid eliminating machines in the first few frames (getCollision() always returns true in the first frames)
                ge[i].fitness -= 1
                cars.pop(i)
                nets.pop(i)
                ge.pop(i)
